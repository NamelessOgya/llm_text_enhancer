...
以下の修正をお願いします。  
### 文字数の制限  
生成物を見ると長めの生成物に高評価がついています。文字数制限を設けたいです。  
単語数がNを超えたら原則score0としてください。
プロンプトにもその旨を明記してください。
文字数制限の設定は`config/task/perspectrum.yaml`に`max_words`として設定し、それを読み込むようにしてください。

### 評価の修正  
SUPPORT / OPPOSEの判定が異なるのに高評価を得ているケースが散見されます。
SUPPORT / OPPOSEの判定が異なる場合は、原則0点としてください。
（単純なMETEORでは判定できないので、新たなルールベース評価機として実装し直してください。）

### LLM as a judgeの実装  
ユーザーの返答内容とLLMの返答内容が主張的にどれだけ近いかを評価する評価機を実装してください。  

### 評価機が異なる場合は別タスク扱いに。  
今回のケースのように、今後一つのタスクを複数の評価機で評価・最適化するケースが出てきます。
`result`上での扱いが同じだと、別評価機で実行した際には上書きされてしまうと思うので、
`result`のフォルダ名に評価機名を含めるようにしてください。
